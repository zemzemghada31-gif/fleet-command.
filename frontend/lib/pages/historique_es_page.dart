...
import '../widgets/qr_scanner_stub.dart'
    if (dart.library.html) '../widgets/web_qr_scanner.dart';
...
class DeliveryLink {
  ...
  DeliveryLink({required this.destinationName, required this.status});
}
...
class EntryExitRecord {
  ...
  String get duration {
    if (exitTime == null) {
      ...
      if (eMatch == null) return 'En cours';
      try {
        final e = DateTime.parse('${eMatch.group(1)}T${eMatch.group(2)}:00');
        final diff = DateTime.now().difference(e);
  ...
  Color get statusColor =>
      status == 'INSIDE' ? const Color(0xFF3B82F6) : const Color(0xFF64748B);
}
...
class _HistoriqueESPageState extends State<HistoriqueESPage> {
  ...
  Map<String, DeliveryLink> _deliveryByPlate = {};
  Timer? _refreshTimer;
  Timer? _clockTimer;

  static List<EntryExitRecord> get _mockRecords => kMockEntryExits
      .map((me) => EntryExitRecord(
            id: me.id,
  ...
  List<EntryExitRecord> get _knownRecords => _records.where((r) {
        ...
        if (r.vehicleModel == 'Inconnu' || r.vehicleModel == 'Sans plaque')
          return false;
        return r.isKnown;
      }).toList();
  ...
  int get _uniqueInsideCount =>
      _uniqueVehicles.where((r) => r.status == 'INSIDE').length;
  int get _uniqueOutsideCount =>
      _uniqueVehicles.where((r) => r.status == 'OUTSIDE').length;

  @override
  void initState() {
    super.initState();
    setState(() {
      _records = List.from(_mockRecords);
      _isLoading = false;
    ...
    try {
      ...
      if (data != null && mounted) {
        final list = (json.decode(data) as List)
            .map((e) => EntryExitRecord.fromJson(e))
            .toList();
        if (list.isNotEmpty) {
          final existingKeys =
              _records.map((r) => '${r.vehiclePlate}_${r.entryTime}').toSet();
          final newRecords = list
              .where((r) =>
                  !existingKeys.contains('${r.vehiclePlate}_${r.entryTime}'))
              .toList();
          if (newRecords.isNotEmpty)
            setState(() {
              _records = [...newRecords, ..._records];
            });
        }
      }
    } catch (_) {}
  ...
  Future<void> _fetchRecords() async {
    ...
    try {
      final res = await http
          .get(Uri.parse('$kApiBaseUrl/api/entry-exit'))
          .timeout(const Duration(seconds: 30));
      if (!mounted) return;
      if (res.statusCode == 200) {
        final serverList = (json.decode(res.body) as List)
            .map((e) => EntryExitRecord.fromJson(e))
            .toList();
      ...
      setState(() => _isLoading = false);
      if (mounted)
        _showSnack('Erreur serveur (${res.statusCode})', Colors.red.shade700);
    } catch (_) {
      if (!mounted) return;
      setState(() {
  ...
  Future<void> _fetchDeliveries() async {
    try {
      ...
      final map = <String, DeliveryLink>{};
      for (final d in list) {
        final plate = d['vehicle_plate'] as String? ?? '';
        if (plate.isEmpty) continue;
        final status = d['status'] as String? ?? '';
        final destName = d['destination_name'] as String? ?? 'Destination';
        final existing = map[plate];
        if (existing == null ||
            (status == 'en_route' && existing.status != 'en_route')) {
          map[plate] = DeliveryLink(destinationName: destName, status: status);
        }
      }
      if (mounted) setState(() => _deliveryByPlate = map);
    } catch (_) {}
  }
  ...
  void _openMobilePlateScanner() {
    final controller =
        MobileScannerController(autoStart: true, facing: CameraFacing.back);
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  child: MobileScanner(
                    ...
                    onDetect: (BarcodeCapture capture) {
                      ...
                      if (barcode?.rawValue == null ||
                          barcode!.rawValue!.isEmpty) return;
                      hasScanned = true;
                      Navigator.of(dialogCtx).pop();
                      controller.dispose();
                      _showAddEntryWithPlate(
              ...
              Padding(
                ...
                child: Column(
                    ...
                    children: [
                      const Text('OU SAISIR LA PLAQUE',
                          style: TextStyle(
                              fontSize: 10,
              ...
              Container(
                ...
                child: cameraFailed
                    ? Center(
                        ...
                        Text(errorMessage ?? 'Caméra non disponible',
  ...
  void _showAddEntryWithPlate(String plate) {
    ...
    final driver =
        kMockVehicles.where((v) => v.plate == plate).firstOrNull?.driver;
    final now = DateTime.now().toString().substring(0, 16);

    _addEntry({
      'vehicle_id':
          kMockVehicles.where((v) => v.plate == plate).firstOrNull?.id ?? 0,
      'vehicle_plate': plate,
      'vehicle_model': model,
      'driver': driver,
      'entry_time': now,
    ...
    if (_backendOffline) {
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('Entrée ajoutée (hors-ligne)'),
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setDialogState) => AlertDialog(
          ...
          title: const Text('Nouvelle entrée E/S',
            ...
            child: SingleChildScrollView(
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                ...
                DropdownButtonFormField<String>(
                  ...
                  items: const [
                    DropdownMenuItem(
                        value: 'INSIDE', child: Text('À l\'intérieur')),
                    DropdownMenuItem(value: 'OUTSIDE', child: Text('Sorti'))
                  ],
                  onChanged: (v) {
                    if (v != null)
                      setDialogState(() {
                        selectedStatus = v;
                        gateCtrl.text = v == 'INSIDE' ? 'Entrée' : 'Sortie';
                      });
                  },
                ),
                const SizedBox(height: 14),
                Container(
                  ...
                  child: Row(children: [
                    Icon(isKnown ? Icons.check_circle : Icons.help_outline,
                        size: 16,
                        color: isKnown
              ...
              onPressed: () {
                if (plateCtrl.text.trim().isEmpty) {
                  ScaffoldMessenger.of(ctx).showSnackBar(const SnackBar(
                      content: Text('Saisissez une immatriculation'),
                ...
                _addEntry({
                  ...
                  'vehicle_model': modelCtrl.text.trim().isEmpty
                      ? 'Inconnu'
                      : modelCtrl.text.trim(),
                  'driver': driverCtrl.text.trim().isEmpty
                      ? null
                      : driverCtrl.text.trim(),
                  'entry_time': entryCtrl.text.trim().isEmpty
                      ? now
                      : entryCtrl.text.trim(),
                  'exit_time': null,
                  'gate': gateCtrl.text.trim().isEmpty
                      ? 'Entrée'
                      : gateCtrl.text.trim(),
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          ListTile(
              dense: true,
              title: const Text('À l\'intérieur'),
              trailing: _filterStatus == 'INSIDE'
                  ? const Icon(Icons.check_circle, color: Color(0xFF0F172A))
                  : null,
              onTap: () {
                setState(() {
                  _filterStatus = 'INSIDE';
                });
                Navigator.pop(context);
              }),
          ListTile(
              dense: true,
              title: const Text('Sorti'),
              trailing: _filterStatus == 'OUTSIDE'
                  ? const Icon(Icons.check_circle, color: Color(0xFF0F172A))
                  : null,
              onTap: () {
                setState(() {
                  _filterStatus = 'OUTSIDE';
                });
                Navigator.pop(context);
              }),
        ]),
  ...
  Widget _buildOfflineBanner() {
    return Container(
      ...
      child: Row(children: [
        ...
        const Expanded(
            child: Text('Backend hors ligne — données locales affichées.',
    ...
    if (isMobile) {
      return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('Historique Entrées / Sorties',
              style: TextStyle(
                  fontSize: 20,
                  fontWeight: FontWeight.bold,
          ...
          Text(
              '${_uniqueVehicles.length} véhicule(s) — $_uniqueInsideCount à l\'intérieur, $_uniqueOutsideCount sortis — ${_deliveryByPlate.length} en livraison${_hideUnknown ? "" : " (incl. inconnus)"}',
        ...
        SingleChildScrollView(
          ...
          child: Row(children: [
            _buildButton('Scanner plaque', Icons.qr_code_scanner,
                onTap: _openPlateScanner, isPrimary: true),
            const SizedBox(width: 8),
            _buildButton('Nouvelle entrée', Icons.add,
  ...
  Widget _buildStatsRow({bool isMobile = false}) {
    ...
    return Row(children: [
      Expanded(
          child: _buildStatCard('À L\'INTÉRIEUR', _uniqueInsideCount.toString(),
              Icons.login, const Color(0xFF3B82F6), isMobile)),
      SizedBox(width: gap),
      Expanded(
          child: _buildStatCard('SORTIS', _uniqueOutsideCount.toString(),
              Icons.logout, const Color(0xFF64748B), isMobile)),
      SizedBox(width: gap),
      Expanded(
          child: _buildStatCard(
              'EN LIVRAISON',
              _deliveryByPlate.length.toString(),
              Icons.route,
              const Color(0xFF10B981),
              isMobile)),
      SizedBox(width: gap),
      Expanded(
          child: _buildStatCard('TOTAL', _uniqueVehicles.length.toString(),
              Icons.swap_horiz, const Color(0xFF0F172A), isMobile)),
    ]);
  }
  ...
  Widget _buildStatCard(String label, String value, IconData icon, Color color,
    ...
    return Container(
      padding: EdgeInsets.symmetric(
          horizontal: isMobile ? 10 : 16, vertical: isMobile ? 10 : 12),
      decoration: BoxDecoration(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Padding(
          ...
          child:
            ...
            Row(children: [
              Text(
                  '${_uniqueVehicles.length} véhicule(s) unique(s)${_filterStatus != null ? " · filtre: ${_filterStatus == "INSIDE" ? "À l\u2019intérieur" : "Sorti"}" : ""}${_hideUnknown ? "" : " · tous"}',
                  style:
            ...
            ? SingleChildScrollView(
                ...
                child: ConstrainedBox(
                  constraints: BoxConstraints(
                      minWidth: math.min(
                          MediaQuery.of(context).size.width - 24, 640)),
                  child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        _buildTableHeader(),
                        ...
                        if (_isLoading)
                          ...
                          ..._uniqueVehicles.map((r) => Column(children: [
                                _buildTableRow(r),
                                const Divider(
                                    color: Color(0xFFF8FAFC), height: 1)
                              ]))
                      ]),
                ),
              )
  ...
  void _showVehicleHistory(EntryExitRecord record) {
    ...
    final totalDuration = completed > 0
        ? vehicleRecords
            ...
            try {
              ...
              if (e != null && x != null) {
                return sum +
                    DateTime.parse('${x.group(1)}T${x.group(2)}:00').difference(
                        DateTime.parse('${e.group(1)}T${e.group(2)}:00'));
              }
            } catch (_) {}
            return sum;
          })
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        contentPadding: EdgeInsets.zero,
        content: SizedBox(
          width: math.min(MediaQuery.of(context).size.width - 32, 560),
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              Container(
                ...
                child: Row(children: [
                  ...
                  Expanded(
                      child: Column(
                        ...
                        Text(
                            '${record.vehicleModel} · ${record.driver ?? "Chauffeur inconnu"}',
                            style: TextStyle(
                                color: Colors.white.withValues(alpha: 0.7),
                                fontSize: 13)),
                        if (_deliveryByPlate
                            .containsKey(record.vehiclePlate)) ...[
                          const SizedBox(height: 4),
                          Row(children: [
                            Icon(Icons.route,
                                size: 12,
                            ...
                            const SizedBox(width: 4),
                            Text(
                                _deliveryByPlate[record.vehiclePlate]!
                                    .destinationName,
                                style: TextStyle(
                                    color: Colors.white.withValues(alpha: 0.7),
                                    fontSize: 11)),
                  ...
                  Column(children: [
                    Container(
                      ...
                      child: Text('$totalTrips trajet(s)',
                          style: const TextStyle(
                              color: Colors.white,
                              fontSize: 12,
                              fontWeight: FontWeight.bold)),
                    ),
                    if (_deliveryByPlate.containsKey(record.vehiclePlate)) ...[
                      const SizedBox(height: 6),
                      _deliveryStatusBadge(
                          _deliveryByPlate[record.vehiclePlate]!.status),
                    ],
                  ]),
                ]),
              ),
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                      ...
                      children: [
                        Row(children: [
                          _historyStat(
                              'Trajets', '$totalTrips', Icons.swap_horiz),
                          const SizedBox(width: 12),
                          _historyStat(
                              'Terminés', '$completed', Icons.check_circle),
                          const SizedBox(width: 12),
                          _historyStat(
                              'Durée totale',
                        ...
                        Row(children: [
                          Expanded(
                            child: GestureDetector(
                              onTap: () {
                                Navigator.pop(ctx);
                                widget.onNavigate?.call(4, record.vehiclePlate,
                                    record.vehicleModel);
                              },
                              child: Container(
                                padding:
                          ...
                          Expanded(
                            child: GestureDetector(
                              onTap: () {
                                Navigator.pop(ctx);
                                widget.onNavigate?.call(5, record.vehiclePlate,
                                    record.vehicleModel);
                              },
                              child: Container(
                                padding:
          ...
          child:
            ...
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              ...
              Container(
                ...
                child: Text(isInside ? 'En cours' : 'Terminé',
                    style: TextStyle(
                        fontSize: 10,
                        fontWeight: FontWeight.bold,
                        color: isInside
            ...
            const SizedBox(height: 2),
            Text('Durée: ${r.duration} · ${r.driver ?? "N/A"}',
  ...
  Widget _buildTableRow(EntryExitRecord r) {
    final dLink = _deliveryByPlate[r.vehiclePlate];
    return GestureDetector(
      ...
      child: Padding(
        ...
        child: Row(children: [
                  ...
                  ? Column(
                      ...
                      children: [
                          Text(dLink.destinationName,
                              ...
                              maxLines: 1),
                          const SizedBox(height: 2),
                          _deliveryStatusBadge(dLink.status),
                        ])
                  : Text('—',
                      style: TextStyle(
          ...
          Expanded(
              flex: 2,
              child: Text(r.entryTime,
                  style:
                      const TextStyle(color: Color(0xFF64748B), fontSize: 12))),
          Expanded(
              flex: 2,
              child: Text(r.exitTime ?? '—',
                  style: TextStyle(
                      color: r.exitTime == null
          ...
          Expanded(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                  ...
                  borderRadius: BorderRadius.circular(6)),
              child: Text(r.status == 'INSIDE' ? "Intérieur" : 'Sorti',
                  style: TextStyle(
                      color: r.statusColor,
                      fontSize: 10,
                      fontWeight: FontWeight.bold),
  ...
  Widget _deliveryStatusBadge(String status) {
    final data = switch (status) {
      'en_route' => (const Color(0xFF3B82F6), 'En route'),
      'arrived' => (const Color(0xFFF59E0B), 'Arrivé'),
      'delivered' => (const Color(0xFF10B981), 'Livré'),
    ...
    return Container(
      ...
      child: Text(data.$2,
          style: TextStyle(
              fontSize: 9, fontWeight: FontWeight.bold, color: data.$1)),
    );
  }
